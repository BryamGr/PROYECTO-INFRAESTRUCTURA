...
resource "aws_ecr_lifecycle_policy" "keep10" {
  for_each   = toset(["auth","inventario","reportes"])
  repository = each.key == "auth" ? aws_ecr_repository.auth.name : each.key == "inventario" ? aws_ecr_repository.inventario.name : aws_ecr_repository.reportes.name
